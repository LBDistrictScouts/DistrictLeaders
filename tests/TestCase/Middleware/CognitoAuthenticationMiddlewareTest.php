 ...
 *
 * @package App\Test\TestCase\Middleware
 */
...
//        $this->application = new Application('config');
    ...
    public function testApplicationAuthentication()
    {
        ...
        static::assertTrue($service->identifiers()->has('Password'));
        static::assertTrue($service->authenticators()->has('Form'));
    }
    ...
    public function testProviderAuthentication()
    {
        ...
        $middleware = new CognitoAuthenticationMiddleware($provider);
        $middleware->process($request, $handler);

        /** @var AuthenticationService $service */
        $service = $handler->request->getAttribute('authentication');
        static::assertInstanceOf(AuthenticationService::class, $service);
        static::assertSame($this->service, $service);
        static::assertSame('identity', $service->getConfig('identityAttribute'));
    ...
    public function testApplicationAuthenticationRequestResponse()
    {
        ...
        $middleware = new CognitoAuthenticationMiddleware($application);
        $middleware->process($request, $handler);
    }
