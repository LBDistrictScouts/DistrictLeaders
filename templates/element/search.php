...
$searchEntity = $this->fetch('entity') ?? '';
$entity = $this->fetch('entity') ?? 'Users';
if (!$entity) {
    $entity = 'Users';
}

if (!empty($searchEntity)) {
    $searchEntity = $this->Inflection->space($searchEntity);
} else {
    $searchEntity = 'Directory';
}
...
<?php if ($this->Identity->isLoggedIn()) : ?>
    <section class="masthead text-white text-center" style="background: url(<?= $this->Url->image('bg-masthead.jpg') ?>)no-repeat center center;background-size: cover;padding: 100px 0px;">
        ...
        <div class="container-fluid" style="margin: 0;">
            <?= $this->Form->create(null, ['type' => 'get', 'url' => ['controller' => $entity, 'action' => 'search'], 'valueSources' => 'query']) ?>
                <div class="row justify-content-center">
                    <div class="col-sm-10 col-md-7 col-lg-7 align-self-center" style="height: 100px;padding: 25px 25px;">
                        <div class="form-group text-white-50">
                            <?= $this->Form->control('q', ['class' => 'form-control form-control-lg flex-fill search-field', 'type' => 'search', 'label' => false, 'placeholder' => 'Search ' . $searchEntity . '...']) ?>
                        </div>
                    </div>
