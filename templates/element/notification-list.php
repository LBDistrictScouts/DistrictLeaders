...
<?php foreach ($notifications as $notification) : ?>
    <tr>
        ...
        <td><small><?= $this->Text->truncate($notification->notification_header, 75) ?></small></td>
        ...
        <td class="actions">
            <?= $this->Identity->buildAndCheckCapability('VIEW', 'Users') ? $this->Html->link($this->Icon->iconHtml('bell'), ['controller' => 'Notifications', 'action' => 'view', $notification->id], ['title' => __('View Notification'), 'class' => 'btn btn-default btn-sm', 'escape' => false]) : '' ?>
            <?= $this->Identity->buildAndCheckCapability('VIEW', 'Users') && isset($index) && $index ? $this->Html->link($this->Icon->iconHtml('user'), ['controller' => 'Users', 'action' => 'view', $notification->user_id], ['title' => __('View User'), 'class' => 'btn btn-default btn-sm', 'escape' => false]) : '' ?>
            <?= $this->Identity->buildAndCheckCapability('VIEW', 'EmailSends') && $notification->has($notification::FIELD_EMAIL_SENDS) && count($notification->email_sends) >= 1 ? $this->Html->link($this->Icon->iconHtml('envelope'), ['controller' => 'EmailSends', 'action' => 'view', $notification->email_sends[0]->id], ['title' => __('View Email Send'), 'class' => 'btn btn-default btn-sm', 'escape' => false]) : '' ?>
        ...
        <td><small><?= $this->Time->format($notification->created, 'dd-MMM-yy HH:mm') ?></small></td>
